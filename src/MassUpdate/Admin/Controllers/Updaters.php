...
class Updaters extends \Admin\Controllers\BaseAuth
{
    ...
    public function index()
    {
        ...
        $this->app->set('meta.title', 'Updaters | Mass Update');
    ...
    private function handleBulkUpdate($selected_model, $where_part, $update_data, $collection)
    {
        $update_part = array();
        $params = array(
            "dataset" => \Base::instance()->get("REQUEST")
        );
        ...
        try
        {
            $collection->update($where_part, $update_part, array(
                "multiple" => true
            ));
    ...
    private function handleDocumentByDocument($selected_model, $where_part, $update_data, $collection)
    {
        ...
        foreach ($cursor as $doc)
        {
            ...
            $collection->update(array(
                '_id' => new \MongoId((string) $model->get('id'))
            ), $model->cast(), array(
                'upsert' => true,
                'multiple' => false
            ));
    ...
    private function processSpecificPart($selected_model, $type)
    {
        ...
        if (count($attr_groups) > 0)
        {
            foreach ($attr_groups as $attr)
            {
                ...
                if (!isset($request[$attr_name . '_' . $type . '_cb']) || is_array($request[$attr_name . '_' . $type . '_cb']) == false || !isset($request[$attr_name . '_' . $type . '_cb'][0]))
                {
    ...
    private function processWherePart($selected_model)
    {
        ...
        if (count($conditions_data) > 0)
        {
            foreach ($conditions_data as $row)
            {
                ...
                if ($row[0]->getNatureOfOperation())
                { // if this operation works with model filter
     ...
     *            with all attributes having operations
     * @param $type Type
    ...
    public function doAjax($f3)
    {
        ...
        $request = $f3->get("POST");
        if (empty($request['model']) || empty($request['group']) || empty($request['attr']) || empty($request['op']) || empty($request['op_type']))
        {
            $res = array(
                'error' => true,
